...
@Component({
  ...
  templateUrl: './ob-participant-personal.component.html',
  styleUrls: ['./ob-participant-personal.component.scss'],
  providers: [
...
export class ObParticipantPersonalComponent implements OnInit {
  ...
  getFormAttributeValues() {
    this._service.getFormAttribute(7, 1).subscribe((res) => {
  ...
  createSampleId(){
    this.splashService.splashScreen({ isLoading : true, message : "SAVING" })
    this._service.createSampleId(7,this.form.value["SampleID"]).subscribe((res)=> {
        this.saveFormId = res;
        this.splashService.splashScreen({isLoading : false, message : "" })
