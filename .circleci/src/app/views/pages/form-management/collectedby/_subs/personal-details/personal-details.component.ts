...
export class collectedbyPersonalDetailsComponent implements OnInit {
 form: FormGroup;
  title: string = "Collected By";
  ...
  ngOnInit() {
    ...
    if (!this.formId)
      this._interactionService._Refid$.subscribe((id) => {
        ...
        this._service.getQuestionnaire(this.formId,21).subscribe((res: any[]) => {
        this.formData = res;
  ...
  createForm() {
    this.form = this.eformFB.group({
      ...
      "firstname": new FormControl({value: '', disabled: this.disableInput},[Validators.required,Validators.maxLength(100)]),
      "lastname": new FormControl({value: '', disabled: this.disableInput},[Validators.required,Validators.maxLength(100)]),
    });
...
this._service.getFormAttribute(21, 1).subscribe((res) => {
this.formAttributes = res;
})

}
prepareForm() {
Object.keys(this.formData[0]).forEach(firstname => {
if (this.form.controls[firstname]) {
  this.form.controls[firstname].patchValue(this.formData[0][firstname], { onlySelf: true });
}
});
...
createSampleId(){
  ...
  this._service.createSampleId(21,this.form.value["firstname"]).subscribe((res)=> {
      this.saveFormId = res;
